...
class CarlaRouteEnv(gym.Env):
    ...
    def __init__(self, host="127.0.0.1", port=2000,
        ...
        self.activate_multi_view = activate_multi_view
        self.eval = eval
        ...
        try:
            ...
            if self.activate_multi_view:
                self.multi_view_cameras = [Camera(self.world, width_mv, height_mv,
                                                  transform=sensor_transforms["cam_front"],attach_to=self.vehicle,
                                                  on_recv_image= lambda e: self._set_front_image(e)),
                                           Camera(self.world, width_mv, height_mv,
                                                  transform=sensor_transforms["cam_front_left"],attach_to=self.vehicle,
                                                  on_recv_image= lambda e: self._set_front_left_image(e)),
                                           Camera(self.world, width_mv, height_mv,
                                                  transform=sensor_transforms["cam_front_right"],attach_to=self.vehicle,
                                                  on_recv_image= lambda e: self._set_front_right_image(e)),
                                           Camera(self.world, width_mv, height_mv,
                                                  transform=sensor_transforms["cam_back"],attach_to=self.vehicle,
                                                  on_recv_image= lambda e: self._set_back_image(e)
                                                  ),
                                           Camera(self.world, width_mv, height_mv,
                                                  transform=sensor_transforms["cam_back_left"],attach_to=self.vehicle,
                                                  on_recv_image= lambda e: self._set_back_left_image(e)),
                                           Camera(self.world, width_mv, height_mv,
                                                  transform=sensor_transforms["cam_back_right"],attach_to=self.vehicle,
                                                  on_recv_image= lambda e: self._set_back_right_image(e)),
                                           ]
    ...
    def reset(self, is_training=False):
        ...
        self.front_left_image_buffer = None
        self.front_right_image_buffer = None
        self.back_image_buffer = None
        self.back_left_image_buffer = None
        self.back_right_image_buffer = None
        self.multi_view_images = [self.front_image_buffer,
                                  self.front_left_image_buffer,
    ...
    def step(self, action):
        ...
        if self.activate_multi_view:
            self.multi_view_images = [self._get_front_image(),
                                      self._get_front_left_image(),
    ...
    def _get_front_image(self):
        while self.front_image_buffer is None:
            pass
        image = self.front_image_buffer.copy()
        self.front_image_buffer = None
        return image

    def _get_front_left_image(self):
        while self.front_left_image_buffer is None:
            pass
        image = self.front_left_image_buffer.copy()
        self.front_left_image_buffer = None
        return image

    def _get_front_right_image(self):
        while self.front_right_image_buffer is None:
            pass
        image = self.front_right_image_buffer.copy()
        self.front_right_image_buffer = None
        return image

    def _get_back_image(self):
        while self.back_image_buffer is None:
            pass
        image = self.back_image_buffer.copy()
        self.back_image_buffer = None
        return image

    def _get_back_left_image(self):
        while self.back_left_image_buffer is None:
            pass
        image = self.back_left_image_buffer.copy()
        self.back_left_image_buffer = None
        return image

    def _get_back_right_image(self):
        while self.back_right_image_buffer is None:
            pass
        image = self.back_right_image_buffer.copy()
        self.back_right_image_buffer = None
        return image
    ...
    def _set_front_image(self, image):
        self.front_image_buffer = image

    def _set_front_left_image(self, image):
        self.front_left_image_buffer = image

    def _set_front_right_image(self, image):
        self.front_right_image_buffer = image

    def _set_back_image(self, image):
        self.back_image_buffer = image

    def _set_back_left_image(self, image):
        self.back_left_image_buffer = image

    def _set_back_right_image(self, image):
        self.back_right_image_buffer = image
